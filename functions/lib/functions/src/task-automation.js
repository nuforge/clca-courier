...
exports.monitorTaskDeadlines = exports.onUserProfileUpdated = exports.onContentCreated = void 0;
...
function getContentType(content) {
    ...
    return contentTypeTag ? (_a = contentTypeTag.split(':')[1]) !== null && _a !== void 0 ? _a : 'news' : 'news';
}
/**
 * Helper function to get required skills for content
 */
function getRequiredSkills(content) {
 ...
 */
async function findAvailableVolunteers(requiredSkills, excludeUserId) {
    try {
 ...
 */
async function getVolunteerWorkload(userId) {
    try {
 ...
 */
function calculateTaskDeadline(category) {
    const now = new Date();
    ...
    return firestore_3.Timestamp.fromDate(deadline);
}
/**
 * Automatic task creation when content is submitted
 * Triggered on new content document creation
 */
exports.onContentCreated = (0, firestore_1.onDocumentCreated)('content/{contentId}', async (event) => {
    ...
    try {
        ...
        if (availableVolunteers.length > 0) {
            ...
            const volunteerScores = await Promise.all(availableVolunteers.map(async (volunteer) => {
        ...
        const now = firestore_3.Timestamp.now();
        const taskFeature = {
        ...
        await snapshot.ref.update({
            ...
            'timestamps.updated': firestore_2.FieldValue.serverTimestamp()
        });
 ...
 */
exports.onUserProfileUpdated = (0, firestore_1.onDocumentUpdated)('userProfiles/{userId}', async (event) => {
    var _a, _b, _c, _d;
    ...
    try {
        ...
        const taskAssignmentsDisabled = ((_c = beforeData.preferences) === null || _c === void 0 ? void 0 : _c.taskAssignments) && !((_d = afterData.preferences) === null || _d === void 0 ? void 0 : _d.taskAssignments);
 ...
 */
async function reassignUserTasks(userId, reason) {
    var _a;
    try {
        ...
        for (const taskDoc of userTasksQuery.docs) {
            ...
            if (availableVolunteers.length > 0) {
                ...
                const workloads = await Promise.all(availableVolunteers.map(async (volunteer) => ({
                    volunteer,
            ...
            const updateData = {
                'features.feat:task.updatedAt': firestore_2.FieldValue.serverTimestamp(),
                'timestamps.updated': firestore_2.FieldValue.serverTimestamp()
            };
            if (newAssignee) {
                ...
                await sendTaskNotification(newAssignee, 'task_reassigned', {
                    ...
                    dueDate: (_a = task.dueDate) === null || _a === void 0 ? void 0 : _a.toDate().toISOString(),
                ...
                updateData['features.feat:task.assignedTo'] = firestore_2.FieldValue.delete();
 ...
 */
exports.monitorTaskDeadlines = (0, scheduler_1.onSchedule)('every day 09:00', async () => {
    var _a, _b;
    try {
        ...
        approachingDeadlinesQuery.docs.forEach(doc => {
            ...
            const task = content.features['feat:task'];
            if (!task || !task.dueDate || !task.assignedTo)
                return;
        ...
        for (const { contentId, content, task } of overdueTasks) {
            ...
            if (task.assignedTo) {
                await sendTaskNotification(task.assignedTo, 'task_overdue', {
                    ...
                    contentTitle: content.title,
                    taskCategory: task.category,
                    dueDate: (_b = task.dueDate) === null || _b === void 0 ? void 0 : _b.toDate().toISOString(),
 ...
 */
async function escalateOverdueTask(contentId, content, task, hoursOverdue) {
    var _a;
    try {
 ...
 */
async function sendTaskNotification(userId, notificationType, notificationData) {
    try {
        ...
        await db.collection('notifications').add({
            ...
            expiresAt: firestore_3.Timestamp.fromDate(new Date(Date.now() + (30 * 24 * 60 * 60 * 1000))) // 30 days
        });
...
//# sourceMappingURL=task-automation.js.map