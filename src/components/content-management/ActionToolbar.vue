      ...
      <div class="row">
        <div class="col-12 col-md-3 q-pa-sm">
          <q-btn color="primary" icon="mdi-refresh"
            ...
            @click="$emit('extract-metadata')" :loading="processingStates.isExtracting" class="full-width" />
        </div>
        <div class="col-12 col-md-3 q-pa-sm">
          <q-btn color="secondary" icon="mdi-text-search"
            :label="(selectedCount || 0) > 0 ? `Extract Text (${selectedCount})` : 'Extract All Text'"
            @click="$emit('extract-text')" :loading="processingStates.isExtractingAllText" class="full-width" />
        </div>
        <div class="col-12 col-md-3 q-pa-sm">
          <q-btn color="accent" icon="mdi-image-multiple"
            :label="(selectedCount || 0) > 0 ? `Generate Thumbs (${selectedCount})` : 'Generate Thumbnails'"
            @click="$emit('generate-thumbnails')" :loading="processingStates.isGeneratingThumbs" class="full-width" />
        </div>
        <div class="col-12 col-md-3 q-pa-sm">
