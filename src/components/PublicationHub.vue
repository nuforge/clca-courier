          ...
          <q-card-section>
            ...
            <div ref="uploadArea" class="upload-area" :class="{ 'upload-area-active': isDragOver }"
              ...
              <div v-else class="upload-content">
                ...
                <q-linear-progress v-model="uploadProgress" color="primary" class="q-mt-md" style="height: 6px" />
              </div>
            </div>
              ...
              <q-list dense>
                <q-item v-for="(step, index) in uploadSteps" :key="index">
                  <q-item-section avatar>
                    <q-avatar :color="step.completed ? 'positive' : 'grey-4'" text-color="white" size="sm">
                      {{ index + 1 }}
                    </q-avatar>
                  </q-item-section>
                  <q-item-section>
        ...
        <q-card class="thumbnails-section q-mb-md">
          <q-card-section>
            ...
            <div class="thumbnail-grid">
              <div v-for="file in pdfFiles" :key="file.id" class="thumbnail-card" @click="previewFile(file)">
                <div class="thumbnail-container">
                  <img v-if="file.thumbnail && !thumbnailErrors[file.id]" :src="file.thumbnail" :alt="file.name"
                    class="thumbnail-image" @error="handleThumbnailError(file.id)" />
                ...
                <div class="thumbnail-details">
                  ...
                  <div class="thumbnail-tags">
                    <q-chip v-for="tag in file.tags" :key="tag" dense color="blue-1" text-color="blue-8" size="sm">
                      {{ tag }}
                    </q-chip>
                  </div>
                </div>
...
// Lifecycle
onMounted(async () => {
  // Clear old cached thumbnails that use blocked URLs
...
async function loadStoredFiles() {
  try {
    const storedFiles = await fileMetadataStorage.getAllFiles()

    // Clean up any files that have old blocked thumbnail URLs
    const cleanedFiles = storedFiles.map(file => {
      if (file.thumbnail && file.thumbnail.includes('drive.google.com/thumbnail?')) {
        // Remove the old thumbnail URL so it gets regenerated
        const cleanFile = { ...file }
        delete cleanFile.thumbnail
        return cleanFile
      }
      return file
    })

    files.value = cleanedFiles
  } catch (error) {
...
async function refreshThumbnails() {
  // Clear thumbnail errors before regenerating
  thumbnailErrors.value = {}

  // Regenerate thumbnails for all PDF files
  await generateThumbnails()

  // Reload files to show updated thumbnails
  await loadStoredFiles()
  await generateThumbnails()
}
...
async function processFiles(selectedFiles: File[]) {
  ...
  for (const file of selectedFiles) {
    try {
      // Simulate upload process delay (in real implementation, this would upload to Google Drive)
      ...
      // Update upload steps
      if (processedFiles === 1) {
        if (uploadSteps.value[0]) uploadSteps.value[0].completed = true
        if (uploadSteps.value[1]) uploadSteps.value[1].completed = true
      }
      if (processedFiles === totalFiles) {
        if (uploadSteps.value[2]) uploadSteps.value[2].completed = true
        if (uploadSteps.value[3]) uploadSteps.value[3].completed = true
      }
