/**
 * Calendar Management Composable
 * Provides reactive calendar state and event management
 */
...
export const useCalendar = () => {
  ...
  const loadEvents = async (loadFilters: CalendarEventFilters = {}) => {
    ...
    try {
      const loadedEvents = await calendarEventsService.getCalendarEvents({
        ...filters.value,
        ...loadFilters,
      });
  ...
  const loadEventsForMonth = async (year: number, month: number) => {
    ...
    try {
      const monthEvents = await calendarEventsService.getEventsForMonth(year, month);
  ...
  const subscribeToEvents = (subscriptionFilters: CalendarEventFilters = {}) => {
    ...
    try {
      eventsSubscription = calendarEventsService.subscribeToCalendarEvents(
