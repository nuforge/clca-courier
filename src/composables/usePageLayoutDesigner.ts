...
export function usePageLayoutDesigner() {
  ...
  const issueContent = computed(() => {
    ...
    return selectedIssue.value.submissions
  ...
  const initializeWithIssue = (issue: NewsletterIssue | null, submissions: ContentDoc[] = []) => {
  ...
  return {
    // State
    selectedIssue: selectedIssue as Ref<NewsletterIssue | null>,
    approvedSubmissions,
    currentTemplate,
    pages,
    contentAreas,
