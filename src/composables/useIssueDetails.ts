...
export function useIssueDetails() {
  ...
  const getIssueById = (id: number): PdfDocument | undefined => {
    return siteStore.archivedIssues.find((issue) => issue.id === id);
  };
  ...
  const getNextIssue = (currentId: number): PdfDocument | undefined => {
    const currentIndex = siteStore.archivedIssues.findIndex((issue) => issue.id === currentId);
  ...
  const getRelatedIssues = (currentIssue: PdfDocument, limit = 3): PdfDocument[] => {
    return siteStore.archivedIssues.filter((issue) => issue.id !== currentIssue.id).slice(0, limit);
  };
  ...
  const issueExists = (id: number): boolean => {
    return siteStore.archivedIssues.some((issue) => issue.id === id);
  };
