                ...
                <div class="col-12">
                    ...
                    <q-card class="q-mb-lg">
                        <q-card-section>
                            <div class="row items-center q-col-gutter-md">
                                ...
                                <div class="col-auto">
                                    <q-btn color="positive" icon="mdi-check-all" label="Bulk Approve"
                                        @click="showBulkApproveDialog" :disable="selectedContent.length === 0" />
                                </div>
                                <div class="col-auto">
                    ...
                    <q-card>
                        <q-tabs v-model="activeTab" dense class="text-grey" active-color="primary"
                            ...
                            <q-tab name="pending" :label="`Pending (${pendingContent.length})`" />
                            <q-tab name="approved" :label="`Approved (${approvedContent.length})`" />
                            <q-tab name="published" :label="`Published (${publishedContent.length})`" />
                            <q-tab name="rejected" :label="`Rejected (${rejectedContent.length})`" />
                        </q-tabs>
                        ...
                        <q-tab-panels v-model="activeTab" animated>
                            ...
                            <q-tab-panel name="pending">
                                <ContentTable :content="pendingContent" :selected="selectedContent"
                                    @update:selected="selectedContent = $event" @approve="approveContent"
                                    @reject="rejectContent" @view="viewContent" show-actions />
                            </q-tab-panel>

                            <!-- Approved Content -->
                            <q-tab-panel name="approved">
                                <ContentTable :content="approvedContent" :selected="selectedContent"
                                    @update:selected="selectedContent = $event" @publish="publishContent"
                                    @unpublish="unpublishContent" @view="viewContent" show-publish-actions />
                            </q-tab-panel>

                            <!-- Published Content -->
                            <q-tab-panel name="published">
                                <ContentTable :content="publishedContent" :selected="selectedContent"
                                    @update:selected="selectedContent = $event" @unpublish="unpublishContent"
                                    @view="viewContent" show-unpublish-actions />
                            </q-tab-panel>

                            <!-- Rejected Content -->
                            <q-tab-panel name="rejected">
                                <ContentTable :content="rejectedContent" :selected="selectedContent"
                                    @update:selected="selectedContent = $event" @reconsider="reconsiderContent"
                                    @view="viewContent" show-reconsider-actions />
                            </q-tab-panel>
                        </q-tab-panels>
                    </q-card>
                </div>
