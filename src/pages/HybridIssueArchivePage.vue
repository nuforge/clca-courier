        ...
        <div class="col-12 col-md-10 col-lg-8">
          ...
          <q-card flat :class="cardClasses" class="q-mb-md">
            <q-card-section>
              ...
              <div v-else class="row q-gutter-md items-end">
                <div class="col">
                  <q-input v-model="searchQuery" placeholder="Search newsletters..." outlined clearable
                    @clear="searchQuery = ''">
                ...
                <div class="col-auto">
                  <q-select v-model="sortBy" :options="sortOptions" label="Sort by" outlined map-options emit-value
          ...
          <div v-if="displayNewsletters.length > 0">
            ...
            <div v-if="viewMode === 'year'">
              <div v-for="(newsletters, year) in sortedNewslettersByYear" :key="year" class="q-mb-lg">
                <div class="text-h5 q-mb-md text-primary">{{ year }}</div>
                <div class="row q-gutter-md">
                  <div v-for="newsletter in newsletters" :key="newsletter.id" class="col-12 col-md-6 col-lg-4">
            ...
            <div v-else-if="viewMode === 'grid'" class="row q-gutter-md">
              <div v-for="newsletter in displayNewsletters" :key="newsletter.id" class="col-12 col-md-6 col-lg-4">
                <HybridNewsletterCard :newsletter="newsletter" />
              </div>
            </div>
          ...
          <q-card v-if="dynamicIssues.issues.value.length > 0" flat :class="cardClasses" class="q-mt-lg">
            <q-card-section>
              <div class="text-h5 q-mb-md">
                ...
                <q-badge color="info" :label="`${dynamicIssues.issues.value.length} issues`" class="q-ml-sm" />
              </div>
              ...
              <div class="row q-gutter-md">
                <div v-for="issue in dynamicIssues.issues.value" :key="issue.id" class="col-12 col-md-6 col-lg-4">
                  <q-card flat bordered class="full-height">
                    ...
                    <q-card-actions>
                      <q-btn flat icon="cloud_download" label="Download" color="info" :href="issue.url || '#'"
                        target="_blank" :disable="!issue.url" />
                      <q-btn flat icon="open_in_new" label="View in Drive" color="secondary"
                        v-if="issue.googleDriveFileId"
                        :href="`https://drive.google.com/file/d/${issue.googleDriveFileId}/view`" target="_blank" />
                    </q-card-actions>
                  </q-card>
                </div>
              </div>
          ...
          <q-card v-else flat :class="cardClasses">
            <q-card-section class="text-center q-pa-xl">
              ...
              <q-btn v-if="searchQuery" flat label="Clear Search" color="primary" @click="searchQuery = ''"
                class="q-mt-md" />
            </q-card-section>
          </q-card>
        </div>
